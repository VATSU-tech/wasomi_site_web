...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Contactez l'école Wasomi. Notre équipe vous répond sous 24h ouvrées." },
      ...
      <section className="py-20 container mx-auto px-4 max-w-7xl">
        <div className="grid lg:grid-cols-5 gap-8">
          ...
          <div className="lg:col-span-2 space-y-4" data-aos="fade-right">
            {[
              { icon: MapPin, title: "Adresse", value: "123 Avenue de l'Éducation, Kinshasa" },
              { icon: Phone, title: "Téléphone", value: "+243 000 000 000" },
              { icon: Mail, title: "Email", value: "contact@wasomi.school" },
            ].map((c) => (
              <div key={c.title} className="p-5 rounded-2xl glass hover:shadow-glow transition-spring flex items-start gap-4">
          ...
          >
            {sent ? (
              <div className="text-center py-12 animate-in zoom-in-95 duration-500">
                ...
                <p className="text-muted-foreground mt-2">Nous revenons vers vous très vite.</p>
...
function Field({ label, name, type = "text", required }: { label: string; name: string; type?: string; required?: boolean }) {
  return (
    <div>
      <label htmlFor={name} className="block text-sm font-semibold mb-2">{label}{required && <span className="text-primary"> *</span>}</label>
      <input
        id={name}
        name={name}
        type={type}
